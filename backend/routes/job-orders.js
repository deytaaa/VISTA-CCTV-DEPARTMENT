...
router.post('/', authMiddleware, (req, res, next) => { if (req.user?.role === 'inventory') return res.status(403).json({ error: 'Forbidden' }); return next(); }, requireAnyRole(['admin']), jobOrderController.create);
router.put('/:id', authMiddleware, (req, res, next) => { if (req.user?.role === 'inventory') return res.status(403).json({ error: 'Forbidden' }); return next(); }, requireAnyRole(['admin']), jobOrderController.update);
